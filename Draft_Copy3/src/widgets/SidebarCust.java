...
//			buttons.add(Sim);
		...
		public Boolean Scroll(long window)
		{
			...
			if(size<8)
			{
			if(glfwGetKey(window, GLFW_KEY_UP) == GL_TRUE && buttons.get(0).getAabb().getMin().y>=buttons1.get(0).getAabb().getMax().y )
			{
			...
			else
			{
				...
				if(glfwGetKey(window, GLFW_KEY_UP) == GL_TRUE &&  buttons1.get(size-1).getAabb().getMin().y<=-300 )
				{
					for(int i = 0; i<size;i++)
						
					{
						guiComponents.get(i).setY(5);
						buttons1.get(i).getAabb().setMin(new Vector2f(guiComponents.get(i).getPosition().x - buttons1.get(i).Width()/2, guiComponents.get(i).getPosition().y - 				buttons1.get(i).Height()/2)); 
						buttons1.get(i).getAabb().setMax(new Vector2f(guiComponents.get(i).getPosition().x + buttons1.get(i).Width()/2, guiComponents.get(i).getPosition().y + 				buttons1.get(i).Height()/2));
						if(buttons1.get(i).getAabb().getMax().y > buttons.get(0).getAabb().getMin().y)
						{
							buttons1.get(i).setZ(1);
						}
					}
				}
				if(glfwGetKey(window, GLFW_KEY_DOWN) == GL_TRUE && buttons.get(0).getAabb().getMin().y<=buttons1.get(0).getAabb().getMax().y)
				{
					for(int i = 0; i<size;i++)
		...
		public void moveUp() {
			...
			if(buttons1.get(size-1).getAabb().getMin().y<-300)
			{
			for(int i = 0; i<size;i++)
				
			{
				
				guiComponents.get(i).setY(70);
				buttons1.get(i).getAabb().setMin(new Vector2f(guiComponents.get(i).getPosition().x - buttons1.get(i).Width()/2, guiComponents.get(i).getPosition().y - buttons1.get(i).Height()/2)); 
				buttons1.get(i).getAabb().setMax(new Vector2f(guiComponents.get(i).getPosition().x + buttons1.get(i).Width()/2, guiComponents.get(i).getPosition().y + buttons1.get(i).Height()/2));
				buttons1.get(i).setZ(1);
				if(buttons1.get(i).getAabb().getMax().y<= buttons.get(0).getAabb().getMin().y && buttons1.get(i).getAabb().getMin().y >=-300)
				{
					buttons1.get(i).setZ(-1);
				}
		...
		{
			int size = guiComponents.size();
			System.out.println("This"+buttons1.get(0).getAabb().getMax().y);
			System.out.println(buttons.get(0).getAabb().getMin().y);
			if(buttons1.get(0).getAabb().getMax().y>buttons.get(0).getAabb().getMin().y)
			{
			for(int i = 0; i<size;i++)
		...
		{	
			...
			float upButtonY = 225;
			Vector3f upButtonPos = new Vector3f(ButtonX, upButtonY, z);
			
			float downButtonY = -250;
			Vector3f downButtonPos = new Vector3f(ButtonX, downButtonY, z);
			
			// add button
			int textureID = loader.loadTexture(UP_BUTTON_TEXTURE_FILE);
			Model upButtonModel = loader.loadToVAO(vertices, texCoords, indices, textureID);
						
			Up = new Button(upButtonModel, upButtonPos, rotation, scale, buttonWidth, buttonHeight);
			
			 textureID = loader.loadTexture(DOWN_BUTTON_TEXTURE_FILE);
			Model downButtonModel = loader.loadToVAO(vertices, texCoords, indices, textureID);
						
			Down = new Button(downButtonModel, downButtonPos, rotation, scale, buttonWidth, buttonHeight);
			
			guiComponents1.add(Up);
			guiComponents1.add(Down);
