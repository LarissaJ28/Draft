...
public class Toolbar {
	...
	private static String SAVE_BUTTON_TEXTURE_FILE = "./res/Save.png";
	private static String DELETE_BUTTON_TEXTURE_FILE = "./res/Delete.png";
	...
	private static String RESET_BUTTON_TEXTURE_FILE = "./res/reset.png";
	...
	public  Toolbar(Loader loader, float screenWidth, float screenHeight, float z) {
		...
		float saButtonX = raButtonX +buttonWidth;
		Vector3f saButtonPos = new Vector3f(saButtonX, ButtonY, z);
		
		float deButtonX = saButtonX +buttonWidth;
		Vector3f deButtonPos = new Vector3f(deButtonX, ButtonY, z);
		
		float stButtonX = deButtonX +buttonWidth;
		Vector3f stButtonPos = new Vector3f(stButtonX, ButtonY, z);
		
		float reButtonX = stButtonX +buttonWidth;
		Vector3f reButtonPos = new Vector3f(reButtonX, ButtonY, z);
		...
		// save button
		textureID = loader.loadTexture(SAVE_BUTTON_TEXTURE_FILE);
		Model saButtonModel = loader.loadToVAO(vertices, texCoords, indices, textureID);
				
		saveButton = new Button(saButtonModel, saButtonPos, rotation, scale, buttonWidth, buttonHeight);
		
		// save button
		textureID = loader.loadTexture(DELETE_BUTTON_TEXTURE_FILE);
		Model deButtonModel = loader.loadToVAO(vertices, texCoords, indices, textureID);
						
		deleteButton = new Button(deButtonModel, deButtonPos, rotation, scale, buttonWidth, buttonHeight);
		
		
		textureID = loader.loadTexture(START_BUTTON_TEXTURE_FILE);
		Model stButtonModel = loader.loadToVAO(vertices, texCoords, indices, textureID);
						
		startButton = new Button(stButtonModel, stButtonPos, rotation, scale, buttonWidth, buttonHeight);
		
		textureID = loader.loadTexture(RESET_BUTTON_TEXTURE_FILE);
		Model reButtonModel = loader.loadToVAO(vertices, texCoords, indices, textureID);
						
		resetButton = new Button(reButtonModel, reButtonPos, rotation, scale, buttonWidth, buttonHeight);
