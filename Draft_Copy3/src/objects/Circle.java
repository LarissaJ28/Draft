...
public class Circle extends Entity {
	...
	public Circle(Model model, Vector3f position, Vector3f velocity, Vector3f acceleration, Vector3f rotation,
			float scale, float mass, float e, float radius, float staticFriction, float kineticFriction, String type) {
		
		super(model, position, velocity, acceleration, rotation, scale, mass, e, 
				staticFriction, kineticFriction, type);
		
		this.radius = radius;
	}
	
	public Circle(Model model, Vector3f position, Vector3f rotation, float scale, float radius2) {
