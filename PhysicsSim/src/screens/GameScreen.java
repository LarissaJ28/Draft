...
public class GameScreen {
	...
	public GameScreen(long window, Loader loader, float screenWidth, float screenHeight, float z,
		...
		for(int i = 1; i<sidebar.getButtons().size();i++)
		{
			...
			System.out.println(sidebar.getButtons().get(i).isEnabled());
		}
		this.loader = loader;
	...
	public void mouseInput(Main main, boolean leftClick, boolean rightClick) {
		...
		if (leftClick) {
			...
			// pop-up box
				...
				else if (popUpBox.getIncreaseSizeButton().getAabb().intersects(x, y)&&popUpBox.getIncreaseSizeButton().isEnabled()) {
				...
				else if (popUpBox.getDecreaseSizeButton().getAabb().intersects(x, y)&&popUpBox.getDecreaseSizeButton().isEnabled()) {
				...
				for (int i = 0; i < sidebar.getButtons().size(); i++) {
					...
					// check if this button was clicked
					if (button.getAabb().intersects(x, y) && sidebar.getButtons().get(i).isEnabled()) {
						
						System.out.println(sidebar.getButtons().get(i).isEnabled());
						for(int n = 1; n<sidebar.getButtons().size();n++)
						{
							
//							sidebar.getButtons().get(i).setEnabled(false);
							System.out.println(sidebar.getButtons().get(n).isEnabled());
						}
