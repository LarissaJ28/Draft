...
public class CustomizedScreen {
	...
	private long window;
	private float z, screenWidth,screenHeight, height,width, index=1;
	...
	public CustomizedScreen(long window, Loader loader, float screenWidth, float screenHeight, float z) {
		...
		guiComponents.addAll(sideBar1.sidec(loader, screenWidth, screenHeight, z));
		buttons = new ArrayList<Button>();
		buttons.addAll(sideBar1.sidec(loader, screenWidth, screenHeight, z));
		this.window=window;
	...
	public void render(Renderer renderer) {
		...
		{
			num2=1;
			ObjectChange.changepos(window, screenWidth,screenHeight,simulation);
			 
		}
		if(num1==3)
		{
			guiComponents = ObjectChange.scrollside(guiComponents, window);
		}
	}
		...
		if (leftClick) {
			...
			for (Button button1: buttons) {
				...
				if (button1.getAabb().intersects(x, y)) {
					...
					// check which button
					if (button1.equals(sideBar1.lev))
					{
					guiComponents.add(sideBar1.sidecAdd(loader, screenWidth, screenHeight, z, index));
					index++;
					}
					
					if (button1.equals(sideBar1.lev1))
						
					num1=3;

					if(button1.equals(sideBar1.lev2))
					{
...
//						float posY = (float) Math.random() * 150;
						...
						float e = -0.5f;
						width=sideLength;
						newRectangle = simulation.createCrateEntity(sideLength, posX, posY, z, mass, e);
				...
				if(num2==1) {
					...
					ObjectChange.down(newRectangle, simulation);
					num1=0;
					num2=0;
//					System.out.println("In");
					
				}
//				else if(num2==1 && inside == false)
//				{
//					num1=0;
//					int size = SimulationWindow.getEntity().size();
//					System.out.println(size);
//					SimulationWindow.delete(size-1);
//				}
			}
