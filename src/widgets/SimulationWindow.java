...
public class SimulationWindow {
	...
	public SimulationWindow(long window, Loader loader, float screenWidth, float screenHeight, float z) {
		...
		x=gX;
		y= lY - gHeight + tHeight;
		height = lHeight;
		width= tWidth-lWidth*2;
	 ...
	 */
	public Rectangle createCrateEntity(float sideLength, float x, float y, float z, float mass, float e) {
		...
		crate.getAabb().setMin(new Vector2f(crate.getPosition().x - crate.getWidth()/2, crate.getPosition().y - crate.getHeight()/2)); 
		crate.getAabb().setMax(new Vector2f(crate.getPosition().x + crate.getWidth()/2, crate.getPosition().y + crate.getHeight()/2));
		
		entities.add(crate);
		return crate;
	}
	
	public static float[] simWindow()
	{
		...
		float[] de;
		return  de = new float[] {xMin,xMax,yMin,yMax};		
	}
	
	public static AABB sim1Window() {
		float xMin = x-width/2;
		float xMax = x+width/2;
		float yMin = y-height/2;
		float yMax = y+height/2;
		
		aabb = new AABB(new Vector2f(xMin, yMin), new Vector2f(xMax, yMax));
		return aabb;
	}
	...
	public void createMetalBoxEntity(float sideLength, float x, float y, float z, float mass, float e) {
		...
		metalBox.getAabb().setMin(new Vector2f(metalBox.getPosition().x - metalBox.getWidth()/2, metalBox.getPosition().y - metalBox.getHeight()/2)); 
		metalBox.getAabb().setMax(new Vector2f(metalBox.getPosition().x + metalBox.getWidth()/2, metalBox.getPosition().y + metalBox.getHeight()/2));
